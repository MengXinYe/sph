...
export default {
  ...
  methods: {
    async open() {
      const result = await import('./images/ewm.jpg')
      this.$alert(`<img src="${result.default}" style="width: 200px; height: 200px" >`, {
      ...
      if (!this.timer) {
        setTimeout(() => {
          this.timer = setInterval(async() => {
            const r = await this.$api.reqPayState(71)
            if (r.code === 200) {
              clearInterval(this.timer)
              this.timer = null
              this.$MessageBox.close()
