    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x"
                v-if="searchParams.categoryName">{{ searchParams.categoryName }}</li>
            <li class="with-x"
            ...
            <li class="with-x"
                ...
                :key="item.split(':')[0]">{{ item.split(':')[1] }}<i @click="removeProp(index)">×</i></li>
          </ul>
        </div>

        <!--selector-->
        <MXSearchSelector @choiceBrand="addChoiceInBread"
                          @propInfo="propInfo" />
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{'active':isOne }"
                    @click="changeState(isOne, '1')">
                  <a>综合<i v-show="isOne"
                       :class="{ 'el-icon-bottom': isDown, 'el-icon-top': isUp }"></i></a>
                </li>
                <li :class="{'active':isTwo }"
                    @click="changeState(isTwo, '2')">
                  <a>销量<i v-show="isTwo"
                       :class="{ 'el-icon-bottom': isDown, 'el-icon-top': isUp }"></i></a>
                </li>
              </ul>
          ...
          <MXPagination :onceShowNum="searchParams.pageSize"
...
<script>
import MXSearchSelector from './SearchSelector/SearchSelector'
...
export default {
  ...
  methods: {
    ...
    addChoiceInBread(trademark) {
      ...
      // 合并query参数
      if (this.$route.query) {
        location.query = this.searchParams
      }
    ...
    propInfo(prop, attr) {
      const content = `${prop.attrId}:${attr}:${prop.attrName}`
      this.searchParams.props.push(content)
    ...
    changeState(target, targteNum) {
      if (target) {
        if (this.isDown) this.searchParams.order = `${targteNum}:asc`
        else this.searchParams.order = `${targteNum}:desc`
      } else {
        this.searchParams.order = `${targteNum}:desc`
      }
      this.getSearchList()
    },
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    this.$nextTick(this.getSearchList)
    next()
  }
}
</script>
